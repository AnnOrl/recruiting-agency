...
passport_1.default.deserializeUser(async function (id, done) {
    ...
    try {
        user = (await currentConnection.getRepository(Users_1.Users).findOne(id)) || false;
    }
    catch (e) {
        err = e;
    }
...
const initPassport = (connection) => {
    ...
    passport_1.default.use(new passport_local_1.Strategy({
        ...
        try {
            const user = await currentConnection.getRepository(Users_1.Users).findOne({ login: username });
            if (!user) {
